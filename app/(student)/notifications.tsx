...
export default function StudentNotificationsScreen() {
  ...
  useEffect(() => {
    console.log('Bildirim Durumu:', {
      kullaniciId: user?.id,
      kullaniciRolu: user?.role,
      tumBildirimler: notifications.length,
      tumBildirimlerDetay: notifications.map(n => ({
        id: n.id,
        userId: n.userId,
        type: n.type,
        title: n.title
    ...
    if (user && notifications) {
      // Öğrenciye ait bildirimleri filtrele
      const userNotifications = notifications.filter(notification => {
        const isMatch = notification.userId === user.id;
      ...
      console.log('Filtrelenmiş Bildirimler:', {
        toplamBildirim: notifications.length,
        filtrelenmis: userNotifications.length,
        ornekBildirim: userNotifications[0]
      });
